...
export default function SearchModal({
    ...
    useEffect(() => {
        if (isOpen && options.fetchUrl) {
            fetchData();
        }
    }, [isOpen, options.fetchUrl]);

    const fetchData = async () => {
        try {
            const response = await axios.get(options.fetchUrl, { withCredentials: true });
    ...
    const filteredItems = useMemo(() => {
        if (!options.filterFunction) return items;
        return items.filter(item => options.filterFunction(item, searchTerm));
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div className="bg-white w-full max-w-md select-none rounded-lg shadow-lg p-6 max-h-[560px] min-h-[560px]">
                <div className='flex justify-between'>
                    ...
                    <IoClose className='w-6 h-6 cursor-pointer' onClick={() => handleOnClose()} />
                </div>
                    ...
                    <CardList>
                        {
                            paginatedItems.length > 0 ? paginatedItems.map((item) => (
                                <Card key={item.id} item={item} onClick={() => handleSelect(item)} renderItem={options.renderItem} />
                            )) : <div className='my-auto text-center'>No results found.</div>
                        }
                    </CardList>
                </div>
                <div className="flex justify-end items-center mt-4 gap-2">
                    ...
                    />
                    <IoIosArrowBack
                        className={`w-9 h-9 text-gray-500 p-2 border border-gray-300 rounded-lg cursor-pointer transform rotate-180 ${(currentPage + 1) * itemsPerPage >= filteredItems.length ? 'bg-gray-200 cursor-not-allowed' : 'hover:bg-gray-200'}`}
...
function CardList({ children }) {
    return <ul className='flex flex-col gap-2 min-h-[350px]'>{children}</ul>;
}

function Card({ item, onClick, renderItem }) {
    return (
        <li className="flex items-center gap p-4 border rounded-md hover:bg-blue-50 border-gray-300 hover:border-blue-500 shadow-sm transition-colors cursor-pointer select-none" onClick={onClick}>
            {renderItem ? renderItem(item) : <DefaultCardContent item={item} />}
        </li>
    );
}
